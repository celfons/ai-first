...
| Design de interface (meta) | `design`, `ui`, `ux`, `tela`, `tipografia`, `token`, `paleta`, `cor`, `layout`, `microcópia`, `acessibilidade`, `a11y`, `design system`, `genérico` | _(este repo-método: `agents/ux-designer.md`, `agents/frontend-engineer.md`)_ | `design-principles.md`, `knowledge.md` (§ régua de UI + anti-padrões) | — | — | `validate-plugin.mjs`, `check-coherence.mjs` |
| Governança/enforcement (meta) | `governança`, `enforcement`, `gate`, `régua`, `fitness`, `invariante`, `trava`, `policy`, `lock`, `mutação`, `fixture`, `codeowners`, `branch protection`, `hook` | _(este repo-método: `scripts/ai-first-fitness.mjs`, `scripts/policy-lock.mjs`, `scripts/fitness-fixtures/`, `hooks/`)_ | `governance/enforcement.md` (§4 fitness · §4b trava), `governance/policy.lock.json` | 0006, 0020 | — | `ai-first-fitness.mjs --verify`, `policy-lock.mjs` |
