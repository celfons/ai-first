...
| **Veredito de gate lido por regex sobre prosa** (`/BLOQUEIA/i` no texto livre do revisor) | *"NÃO BLOQUEIA"* casa com a regex e dispara re-run falso; um bloqueio fraseado de outro jeito (*"REPROVADO"*) não casa e **passa direto ao merge** | o ponto de decisão mais crítico do pipeline vira loteria de fraseado — falso bloqueio queima orçamento, **falso verde fura o gate** (P-11), e no modo `autônomo` não há humano depois para pegar | veredito **estruturado por `schema`** (`aprova\|bloqueia` + confiança + achados) e o script roteando pelo **campo**, não pelo texto (ADR-0019 — o mesmo remédio do ADR-0017, aplicado à decisão) |
